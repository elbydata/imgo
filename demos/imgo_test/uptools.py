...
def img_to_df(base_path):
    ...
    if class_list == []:
        img_list = [f"{base_path}/{f}" for f in os.listdir(base_path) if not f.startswith(".")]
...
def display_img_df(df, batch_no, batch_size, n_rows, n_cols):
    ...
    if n_rows * n_cols != batch_size:
        raise Exception(f"Cannot display {batch_size} images in {n_rows} rows and {n_cols} cols.")

    batches = np.divmod(len(df), batch_size)[0] + bool(np.divmod(len(df), batch_size)[1])

    if (batch_no + 1) > batches:
        raise Exception(f"'batch_no' argument out of range; final batch is {batches-1}.")
...
class Image_Dataset:
    ...
    def __init__(self,
        ...
        if type(img_scale) is int:
            ...
            raise Exception(f"'img_scale' argument must be integer, {type(img_scale)} given.")
            rescale_dims = None

        if normalize and standardize:
            raise Exception("Can either normalize or standardize data, cannot do both.")
            self.reduce = None
        ...
        if manual_classes is not None:
            if mode == "imgs":
                raise Exception("'manual_classes' argument can only be given for 'np' and 'h5' modes.")
                self.class_list = None
                self.class_no = None
            else:
                if type(manual_classes) is list:
                    self.class_list = sorted([str(i) for i in manual_classes],key=lambda f: f.lower())
                    self.class_no = len(self.class_list)
                else:
                    raise Exception(f"'manual_classes' argument must be list, {type(manual_classes)} given.")
                    self.class_list = None
                    self.class_no = None
        else:
            if self.mode == "imgs":
        ...
        if (self.mode == "np") or (self.mode == "h5"):
            ...
            for r,d,f in os.walk(base_path):
                for i in f:
                    if not i.startswith("."):
                        file_list.append([i,os.path.relpath(os.path.join(r,i))])
            ...
            for file in file_list:
                if file[0].lower().endswith(".npz"):
                    ...
                    for k,v in combo_sets.items():                    
                        if file[0].lower().startswith(f"x_{k}".lower()):
                            combo_sets[k][0] = np.load(file[1], allow_pickle=True)
                        elif file[0].lower().startswith(f"y_{k}".lower()):
                            combo_sets[k][1] = np.load(file[1], allow_pickle=True)
                    ...
                    raise Exception("No valid '.npy', '.npz', or '.h5' files identified.")
                    
            if class_list_pending:
                y_shapes = {}
                for v in [v for k,v in combo_sets.items() if (v[1] is not None)]:
                ...
                if len(set(y_shapes.values())) == 1:
                    ...
                    for i in range(1,self.class_no+1):
                        len_dif = len(str(self.class_no))-(len(c+str(i)) - len(str(self.class_no)))
                        cl = (str(0)*len_dif)+str(i)
                        clist.append(f"class_{cl}")                    
                    self.class_list = clist
                else:
                    raise Exception("Inconsistent number of classes in y-arrays.")                                  
            ...
            for i, j in tqdm(data_df.iterrows(),total=len(data_df),desc="Reading images",position=0):
        ...
        self.split = int(len([1 for k,v in combo_sets.items() if v[0] is not None])-1) 
        
        x_eqdims = []
        for k,v in combo_sets.items():
            if v[0] is not None:
                if (len(v[0].shape) == 4) and (v[0].shape[1] == v[0].shape[2]):
        ...
        if pre_norm and pre_std:
            raise Exception("Cannot expand data if both pre-normalized and pre-standardized.")
            self.expand = None
            ...
            if self.dims == "various":
                raise Exception("Cannot expand data if image dimensions are not the same.")
            else:
                if self.split == 0:
        ...
        for k,v in X_sets.items():
            if combo_sets[k][0] is not None:
                for i in tqdm(np.arange(combo_sets[k][0].shape[0]),total=combo_sets[k][0].shape[0],desc=f"Processing X_{k}"):
                    
                    if self.expand is not None:
                        if self.expand == "de_std":
                            img_min = np.min(combo_sets[k][0][i])
                            img_max = np.max(combo_sets[k][0][i])
                            raw_img = (((combo_sets[k][0][i] - img_min)*255)/(img_max - img_min)).astype(np.uint8)
                        elif self.expand == "de_norm":
                            raw_img = (combo_sets[k][0][i] * 255).astype(np.uint8)
                        else:
        ...
        if self.reduce == "std":
            if self.dims == "various":
                raise Exception("Cannot standardize data if image dimensions are not the same.")
            else:
                if self.split == 0:
                    print("Data will be standardized when split using 'data_split'.")
                    for k,v in combo_sets.items():
                        if v[0] is None:
                    ...
                    print("Standardizing...")
                    self.mu = np.sum(combo_sets["train"][0])/combo_sets["train"][0].size
                    self.sigma = np.sqrt(np.sum((combo_sets["train"][0]-self.mu)**2)/combo_sets["train"][0].size)
                    for k,v in combo_sets.items():
                        if v[0] is None:
                            setattr(self, f"X_{k}", None)
                        else:
                            setattr(self, f"X_{k}", (v[0]-self.mu)/self.sigma)
                        if v[1] is None:
                            setattr(self, f"y_{k}", None)
                        else:
                            setattr(self, f"y_{k}", v[1]) 
...
#     ----------
    ...
    def details(self, plot=False):
        ...
        ds_dict = {"total_images": self.size,
                   "splits": splits,
        ...
        if plot:
            ...
            fig.text(0,0.9,"Class",fontsize=15,fontweight="black",color="#333F4B")

            ldf.plot.barh(stacked=True, ax=ax, color=colors)

            ax.set_xlabel("Images",fontsize=15,fontweight="black",color="#333F4B")
            ax.set_ylabel("")
...
#     ----------
    ...
    def map_classes(self,class_list):
        ...
        if type(class_list) is list:    
            new_list = sorted([str(i) for i in class_list],key=lambda f: f.lower())

            if len(new_list) == len(self.class_list):
                for i in range(len(self.class_list)):
                    self.class_list[i] = new_list[i] 
            else:
                raise Exception("Number of classes given does not match number in Image_Dataset.")  
        else:
            raise Exception(f"'class_list' argument must be list, {type(class_list)} given.")
        ...
        if self.split != 0:
            ...
            if type(split_ratio) is tuple:
                if sum(split_ratio) == 1:
                    ...
                    if len(split_ratio) == 3:
                        if stratify:
                            (self.shadow["train"][0], Xtv, self.shadow["train"][1], ytv) = train_test_split(self.shadow["data"][0],self.shadow["data"][1],test_size=round(split_ratio[1] + split_ratio[2], 1),stratify=self.shadow["data"][1],random_state=seed)
                            (self.shadow["val"][0], self.shadow["test"][0], self.shadow["val"][1], self.shadow["test"][1]) = train_test_split(Xtv,ytv,test_size=split_ratio[2]/ round(split_ratio[1] + split_ratio[2], 1),stratify=ytv,random_state=seed)
                        else:
                            (self.shadow["train"][0], Xtv, self.shadow["train"][1], ytv) = train_test_split(self.shadow["data"][0],self.shadow["data"][1],test_size=round(split_ratio[1] + split_ratio[2], 1),random_state=seed)
                            (self.shadow["val"][0], self.shadow["test"][0], self.shadow["val"][1], self.shadow["test"][1]) = train_test_split(Xtv,ytv,test_size=split_ratio[2]/ round(split_ratio[1] + split_ratio[2], 1),random_state=seed)                          
                        del Xtv
                        del ytv
                        ...
                        if stratify:
                            (self.shadow["train"][0], self.shadow["test"][0], self.shadow["train"][1], self.shadow["test"][1]) = train_test_split(self.shadow["data"][0],self.shadow["data"][1],test_size=split_ratio[1],stratify=self.shadow["data"][1],random_state=seed)
                        else:
                            (self.shadow["train"][0], self.shadow["test"][0], self.shadow["train"][1], self.shadow["test"][1]) = train_test_split(self.shadow["data"][0],self.shadow["data"][1],test_size=split_ratio[1],random_state=seed)                        
                        self.shadow["data"][0] = None
                        self.shadow["data"][1] = None  
                        ...
                        raise Exception("'split_ratio' argument must be tuple of 2 or 3 floats.")
                else:
                    raise Exception("'split_ratio' argument must sum to 1.")                        
            else:
                raise Exception(f"'split_ratio' argument must be tuple, {type(split_ratio)} given.")
            ...
            if self.reduce == "std":
                if self.dims == "various":
                    ...
                    self.sigma = np.sqrt(np.sum((subsets["train"][0]-self.mu)**2)/subsets["train"][0].size)
            ...
            if self.split == 1:
                print("Data sucessfully split into training and testing subsets.")
            elif self.split == 2:
                print("Data sucessfully split into training, validation, and testing subsets.")
...
#     ----------        
    ...
    def data_merge(self):
        ...
        if self.split == 0:
            ...
            print("Merging...")
            x_merge = [v[0] for k,v in self.shadow.items() if (v[0] is not None)]
            y_merge = [v[1] for k,v in self.shadow.items() if (v[1] is not None)]
            try:
                self.shadow["data"][0] = np.concatenate([v[0] for k,v in self.shadow.items() if (v[0] is not None)])
                self.shadow["data"][1] = np.concatenate([v[1] for k,v in self.shadow.items() if (v[1] is not None)])
            except:
                merge_x_listwise = []
                merge_y_listwise = []
                for i in x_merge:
            ...
            if self.reduce == "std":
                if self.dims == "various":
                    ...
                    self.min_pv = ((self.min_pv*self.sigma)+self.mu).astype(np.uint8) 
                    self.max_pv = ((self.max_pv*self.sigma)+self.mu).astype(np.uint8)                               
...
#     ----------            
    ...
    def display_batch(self, n_rows, n_cols):
        ...
        if n_rows * n_cols > self.size:
            raise Exception(f"Cannot display {n_rows*n_cols} images because only {self.size} in dataset.")
        else:
    ...
    def save_imgs(self, save_dir):
        ...
        for k,v in self.shadow.items():
            if (v[0] is not None) and (v[1] is not None):
                for i in tqdm(np.arange(v[1].shape[0]),total=v[1].shape[0],desc="Saving"):
                    img = v[0][i]
                    label_index = np.argmax(v[1][i], axis=0)
                    label = self.class_list[label_index]
                    class_counter[label] += 1
                    path = my_path+"/"+label
                    imageio.imwrite(f"{path}/{label}_{class_counter[label]}.jpg",img)
    ...
    def augment_training_set(self, augmenter, portion, inplace=False):
        ...
        if self.split == 0:
            ...
            if (portion >= 0) and (portion <= 1):
                n = np.round(self.X_train.shape[0] * (portion)).astype(np.uint8)
                img_indices = np.random.choice(self.X_train.shape[0],np.min([self.X_train.shape[0], n]),replace=False)
            else:
            ...
            if self.reduce == "norm":
                if inplace:
                    self.shadow["train"][0] = np.array(X_train_aug)/255
